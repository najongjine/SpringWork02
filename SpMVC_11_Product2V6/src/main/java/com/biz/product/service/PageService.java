...
/*
 * totalCount값과 currentPageNo 값으로 pagination에 사용할 여러 변수 값들을 생성.
 */
@Slf4j
@Service
public class PageService {
	/*
	 * pagination을 시작할때 한페이지에 보여질 기본값을 설정
	 */
	...
	public PageDTO getPagination(long totalCount,int currentPageNo) {
		...
		PageDTO pageDTO=PageDTO.builder()
				...
				.listPerPage(listPerPage).offset(offset)
				.limit(limit)
				.firstPageNo(1).finalPageNo(finalPageNo)
				.startPageNo(startPageNo).endPageNo(endPageNo).currentPageNo(currentPageNo)
				.prePageNo(prePageNo).nextPageNo(nextPageNo)
				.build();
