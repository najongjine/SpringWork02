...
public class BokListService {
	...
	public String getQueryHeader(BokSearchDTO bokDTO) {
		...
		/*if(bokDTO.getCallTp().equalsIgnoreCase("L")) {
			queryString+="&NationalWelfarelist="+bok_list_url;
		} else {
			queryString+="&NationalWelfaredetailed="+bok_detail_url;
		}
		*/
	...
	public List<BokListVO> getRestResult(BokSearchDTO bokDTO) {
		...
		//resttemplate을 사용하여 api에 보낼 uri 생성
		URI restURI=null;
		...
		try {
			...
			//restURI를 서버에 보내서 수신된 xml형 문자열을 wantedList 클래스에 선언된 형식에
			//맞추어서 객체로 만들어라(아주 중요)
			...
			//body에는 wantedList클래스형 데이터가 들어있다.
