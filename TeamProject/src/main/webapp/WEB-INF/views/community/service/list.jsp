...
#lee-community-nav>ul>li:last-child>a {
	background-color: #e67e22;
	color: white;
}

#lee-community-nav>ul>li:last-child {
	margin-right: 10px;
}
...
<div id= "lee-community">
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach items="${SELIST}" var="seDTO" varStatus="index">
				<tr class="lee-content-body" data-id="${seDTO.se_seq}">
					<td class="lee-community-list-td">${seDTO.se_seq}</td>
					<td id="lee-community-subject" class="lee-community-list-td">${seDTO.se_subject}</td>
					<td class="lee-community-list-td">${seDTO.se_auth}</td>
					<td class="lee-community-list-td">${seDTO.se_date}</td>
				</tr>
			</c:forEach>
		</c:otherwise>
	</c:choose>
	...
	<%@ include file="/WEB-INF/views/community/service/pagination.jsp" %>
